...
try {
  ...
  check(await chargerJusqua('mydiag-v9-6'), 'page exécute bien v9-6');
  ...
  fs.writeFileSync(SW, swOrig.replace("'mydiag-v9-6'", "'mydiag-v9-7'"));
  await page.reload(); await page.waitForTimeout(2000); await page.reload(); await page.waitForTimeout(2000);
  check(await visible(), 'worker v9-7 alors que la page est en v9-6 : bandeau affiché');
  ...
  fs.writeFileSync(APP, appOrig.replace("'mydiag-v9-6'", "'mydiag-v9-7'") + '\nwindow.CVT = CACHE_VERSION;\n');
  check(await chargerJusqua('mydiag-v9-7'), 'page passée en v9-7 après rechargement');
