...
try {
  ...
  check(await chargerJusqua('mydiag-v9-5'), 'page exécute bien v9-5');
  ...
  fs.writeFileSync(SW, swOrig.replace("'mydiag-v9-5'", "'mydiag-v9-6'"));
  await page.reload(); await page.waitForTimeout(2000); await page.reload(); await page.waitForTimeout(2000);
  check(await visible(), 'worker v9-6 alors que la page est en v9-5 : bandeau affiché');
  ...
  fs.writeFileSync(APP, appOrig.replace("'mydiag-v9-5'", "'mydiag-v9-6'") + '\nwindow.CVT = CACHE_VERSION;\n');
  check(await chargerJusqua('mydiag-v9-6'), 'page passée en v9-6 après rechargement');
